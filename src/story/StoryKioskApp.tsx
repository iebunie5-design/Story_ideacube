...
export function StoryKioskApp() {
  ...
  async function createStory() {
    ...
    setPageIndex(0);
    setSceneImages({});
    const result = await generateStory(selection);
    setStory(result);
    setStep("result");

    // 결과 화면 전환 후 핵심 4장면 이미지를 병렬로 백그라운드 생성
    Promise.allSettled(
      IMAGE_PAGES.map(async (idx) => {
        const scene = result.pages[idx];
        if (!scene) return;
        const image = await generateSceneImage(selection, scene, idx);
        if (image) {
          setSceneImages((prev) => ({ ...prev, [idx]: image.imageDataUrl }));
        }
      })
    );
  }
  ...
  function printStorybook() {
    ...
    const printImages = Object.fromEntries(
      Object.entries(sceneImages).map(([key, value]) => {
      ...
      <section className={["relative grid min-h-screen p-6 sm:p-8 lg:p-12", step === "attract" ? "grid-rows-1" : "grid-rows-[auto_1fr]"].join(" ")}>
        ...
        {step === "attract" ? (
          <button
            ...
            className="z-10 grid min-h-0 place-items-center py-5 text-center focus:outline-none"
            aria-label="동화 만들기 시작"
          >
            <span className="flex flex-col items-center gap-8">
              <span className="text-[clamp(64px,12vw,180px)] font-black leading-none text-white drop-shadow-[0_0_34px_rgba(125,232,255,0.34)]">
                동화 만들기
              </span>
              <span aria-hidden="true" className="story-mascot-float block h-[clamp(170px,24vw,330px)] w-[clamp(170px,24vw,330px)]">
                <img
                  src="/images/buyeo-mascot-transparent.png"
                  alt=""
                  className="h-full w-full object-contain drop-shadow-[0_22px_34px_rgba(0,0,0,0.32)]"
                  draggable={false}
                />
              </span>
            </span>
            ...
            {step === "result" ? (
              <div className="flex h-full flex-col gap-5">
                ...
                <StoryTextPanel
                  story={story}
                  pageIndex={pageIndex}
                  imageReady={!IMAGE_PAGES.includes(pageIndex as typeof IMAGE_PAGES[number]) || Boolean(sceneImages[pageIndex])}
                  onPrev={() => setPageIndex((current) => Math.max(0, current - 1))}
                  onNext={() => setPageIndex((current) => Math.min(story.pages.length - 1, current + 1))}
                />
                <div className="mt-auto grid grid-cols-1 gap-3 border-t border-[#73DFFF]/20 pt-4 sm:grid-cols-2">
                  <PrimaryButton onClick={reset}>
                    새로 만들기 <SparklesIcon className="h-6 w-6" />
                  </PrimaryButton>
        ...
        {step === "result" ? (
          <div className="z-10 mx-auto grid w-full max-w-[1180px] grid-cols-2 gap-4 pb-2">
            <SecondaryButton onClick={downloadStorybook} disabled={!printableImagesReady}>
              <ArrowDownTrayIcon className="h-5 w-5" /> {printableImagesReady ? "완성본 다운로드" : "이미지 준비 중"}
            </SecondaryButton>
            <SecondaryButton onClick={printStorybook} disabled={!printableImagesReady}>
              <PrinterIcon className="h-5 w-5" /> 인쇄하기
            </SecondaryButton>
          </div>
        ) : null}
        </>
